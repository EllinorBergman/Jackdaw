...
is_null($saveAs) or in_array($saveAs, array('png', 'jpg', 'jpeg', 'gif')) or $img->errorMessage('Not a valid extension to save image as'); 
...
if($verbose) { $img->verbose("Cache file is: {$cacheFileName}"); } 
...
// 
if($verbose) { $img->verbose("File extension is: {$fileExtension}"); } 

switch($fileExtension) {   
  case 'jpg': 
  case 'jpeg':  
    $image = imagecreatefromjpeg($pathToImage); 
    if($verbose) { $img->verbose("Opened the image as a JPEG image."); } 
    break;   
   
  case 'png':   
    $image = imagecreatefrompng($pathToImage);  
    if($verbose) { $img->verbose("Opened the image as a PNG image."); } 
    break;   
